...
class ZeypherMainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        ai_conn_grp = QGroupBox("AI Engine")
        ag = QGridLayout()
        ag.setSpacing(4)
        ag.addWidget(QLabel("Decart API Key:"), 0, 0)
        ...
        btn_row.addWidget(self.btn_lucy)
        if HAS_FAL:
        ...
        saas_grp = QGroupBox("SaaS Billing (optional)")
        sg = QVBoxLayout(saas_grp)
        sg.setSpacing(4)
        saas_row = QHBoxLayout()
        saas_row.addWidget(QLabel("Server:"))
        self.saaS_url = QLineEdit("http://localhost:8000")
        self.saaS_url.setPlaceholderText("URL")
        saas_row.addWidget(self.saaS_url, stretch=1)
        sg.addLayout(saas_row)
        saas_row2 = QHBoxLayout()
        self.saaS_user = QLineEdit()
        self.saaS_user.setPlaceholderText("Username")
        saas_row2.addWidget(self.saaS_user, stretch=1)
        self.saaS_pass = QLineEdit()
        self.saaS_pass.setEchoMode(QLineEdit.Password)
        self.saaS_pass.setPlaceholderText("Password")
        saas_row2.addWidget(self.saaS_pass, stretch=1)
        sg.addLayout(saas_row2)
        self.btn_saaS_login = QPushButton("Login")
        self.btn_saaS_login.clicked.connect(self._saas_login)
        sg.addWidget(self.btn_saaS_login)
        self.saaS_status = QLabel("")
        self.saaS_status.setStyleSheet("color: #888; font-size: 11px;")
        sg.addWidget(self.saaS_status)
        saas_grp.setLayout(sg)
        right_col.addWidget(saas_grp)
    ...
    def _toggle_fal(self):
        ...
        try:
            if self.fal_engine and self.fal_engine.connected:
                ...
                if self.fal_engine is None:
                    self.fal_engine = FalFaceSwap()
                self.fal_engine.set_model("face-swap")
                ...
                if self.fal_engine.connect(key):
                    self.fal_status.setText("Connected")
